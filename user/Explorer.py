...
class BlockchainExplorer:
    ...
    def fetch_blockchain_data(self):
        ...
        try:
            print("📡 Fetching blockchain data from node...")
            ...
            if response.status_code == 200:
                ...
                print(f"✅ Loaded {self.blockchain_length} blocks with {self.total_transactions} transactions")
                return True
            else:
                print(f"❌ Error fetching blockchain: {response.text}")
                return False
...
def main():
    ...
    if not explorer.fetch_blockchain_data():
        print("❌ Failed to connect to node. Please check NODE_URL in .env")
        return
