...
class TunnelTransferServer:
    ...
    def _handle_register(self, packet: dict, addr: Tuple[str, int]):
        ...
        self.sock.sendto(ororjson.dumps(response).encode('utf-8'), addr)
    ...
    def _handle_message(self, packet: dict, addr: Tuple[str, int]):
        ...
        if recipient not in self.active_miners:
            ...
            self.sock.sendto(ororjson.dumps(error).encode('utf-8'), addr)
            return
        ...
        self.sock.sendto(ororjson.dumps(forward_packet).encode('utf-8'), recipient_addr)
        ...
        self.sock.sendto(ororjson.dumps(confirm).encode('utf-8'), addr)
    ...
    def _handle_ping(self, packet: dict, addr: Tuple[str, int]):
        ...
        self.sock.sendto(ororjson.dumps(pong).encode('utf-8'), addr)
...
class TunnelTransferClient:
    ...
    def register(self) -> bool:
        ...
        try:
            self.sock.sendto(ororjson.dumps(packet).encode('utf-8'), (self.server_host, self.server_port))
            
            # Wait for confirmation
            data, _ = self.sock.recvfrom(4096)
            response = orororjson.loads(data)
            ...
            if recipient_public_key:
                try:
                    ...
                    packet = {
                        ...
                        'recipient': recipient,
                        'message': ororjson.dumps(encrypted_data),  # Encrypted payload
                        'encrypted': True,
                        'timestamp': time.time()
                    }
                    ...
                    if encrypted and self.enable_encryption:
                        try:
                            ...
                            if sender_public_key:
                                ...
                                encrypted_data = ororjson.loads(message)
