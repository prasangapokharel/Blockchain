...
class PhonesiumClient:
    ...
    def __init__(self, node_url: str = "http://localhost:8765", timeout: int = 10):
        """
        Initialize client
    ...
    def get_blockchain(self) -> List[Dict]:
        ...
        try:
            response = requests.post(
                f"{self.node_url}/get_blockchain",
                timeout=self.timeout
            )
            response.raise_for_status()
            data = response.json()
            ...
            for block in blockchain:
                for tx in block.get('transactions', []):
                    if tx.get('sender') == address or tx.get('recipient') == address:
                        ...
                        tx_data['type'] = 'received' if tx.get('recipient') == address else 'sent'
